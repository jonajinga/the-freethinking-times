---
layout: article
title: "The Pentagon Papers at Fifty-Plus: What We Actually Learned"
description: "The publication of the Pentagon Papers is remembered as a triumph of press freedom. The deeper lesson — about institutional deception and its durability — is less often drawn."
section: History
author: jon-ajinga
authorName: The Editors
date: 2026-02-15
edition: 1
tags:
  - history
  - journalism
  - government
  - Vietnam
  - press freedom
corrections:
  - date: "2026-03-10"
    description: "An earlier version of this article stated that the Pentagon Papers study was commissioned in 1966. It was commissioned in 1967. The date has been corrected in the text."
  - date: "2026-03-14"
    description: "The article originally described the Supreme Court vote as 7-2. The correct vote was 6-3. We regret the error."
---

The Pentagon Papers case is taught, correctly, as a landmark in the history of press freedom. The Supreme Court's decision{% fn 1 %}*New York Times Co. v. United States*, 403 U.S. 713 (1971). The decision was 6-3, with each justice filing a separate opinion. Justice Hugo Black's concurrence remains one of the most forceful statements of First Amendment absolutism in Supreme Court history: "The press was protected so that it could bare the secrets of government and inform the people."{% endfn %} that the government could not impose prior restraint on the publication of the classified study — over the explicit objection of the Nixon administration — established a principle that remains foundational to the legal protection of investigative journalism in the United States.

This is the version of the story that gets told. It is true. It is also incomplete in ways that matter.

## What the Papers Actually Showed

The Pentagon Papers were not, primarily, a story about Vietnam. They were a story about how governments manage information — specifically, about the systematic gap between what officials knew and what they told the public and Congress.

The study, commissioned by Secretary of Defense Robert McNamara{% fn 2 %}McNamara commissioned the study in 1967 with the explicit intent of creating an accurate historical record for future administrations. He later wrote in his memoir *In Retrospect* (1995) that he had underestimated how damning the conclusions would prove to be: "I want to say, and I say it with a deep sense of pain and regret, that I was wrong, terribly wrong."{% endfn %} and completed in 1969, documented that multiple administrations had known that the war was not winnable under existing conditions, had made decisions they understood to be unlikely to achieve stated objectives, and had continued to present the public with an optimistic picture that the internal record did not support.

The officials involved were not, in the main, cynics or liars in any simple sense. Many of them believed in the importance of what they were doing. The gap between the public and private record was produced not by individual dishonesty but by institutional dynamics: the pressures of the policy process, the culture of optimism that attached to official statements, the reluctance to acknowledge failure in ways that would constrain future options.

## The Durability of the Pattern

What is striking, from a distance of more than fifty years, is how durable this pattern has proven. The specific mechanisms documented in the Pentagon Papers — the gap between classified assessment and public statement, the management of information to preserve policy options, the institutional optimism bias in official communication — have appeared, with variations, in subsequent episodes of the kind that historians eventually document in retrospect.

This is not because officials are uniquely dishonest. It is because the incentive structures that produced the behavior in the Pentagon Papers case are not unique to that moment. They are structural features of how governments manage information in democratic systems where public opinion matters and failure carries political cost.

## What Journalism Can and Cannot Do

The Pentagon Papers case is also a story about the limits of journalism as a check on institutional deception. Daniel Ellsberg leaked the papers in 1971.{% fn 3 %}Before approaching the *New York Times*, Ellsberg had spent more than two years attempting to interest sitting senators — including George McGovern and J. William Fulbright — in the papers, hoping they might be entered into the Congressional Record. None were willing to take the political risk. He turned to the press after these efforts failed entirely.{% endfn %} The events they documented had largely concluded years earlier. Journalism revealed what had happened; it did not prevent it while it was happening.

This is the more uncomfortable lesson of the case, and the one least often drawn. The press as a check on government power is a real and important function — but it is a retrospective check, operating primarily on events that have already occurred, against institutions that have strong incentives and significant resources to shape the information environment in real time.

Understanding this clearly is a prerequisite for thinking carefully about what journalism can actually accomplish, and what other mechanisms of accountability it must work alongside.
